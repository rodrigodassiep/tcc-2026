...
\hypersetup{
  ...
  pdfsubject={Proposal for a undergraduate thesis about Visualizing Responsibility Distribution in the Linux Kernel Codebase},
}
...
\par All these tools aim to simplify the process of analyzing large codebases and to help intelligently manage these complex work hierarchies, which is, as previously stated, one of the biggest challenges when dealing with open source software.
\section{Proposal}
\subsection{Objectives}
Aiming to provide another tool to visualize open source codebases, the Linux Kernel in specific, this work objectives are:
\begin{itemize}
	\item Evaluate the previously developed tools and how they could aggregate to our work.
	\item Develop an improved interactive Tree-Map \cite{treemap} visualization of the Kernel, displaying the responsability distribution of the kernel codebase.
	\item Make the implementation more efficient, fluid and user-friendly, removing the sttutering present in some of the old visualizations.
\end{itemize}
...
\begin{itemize}
	...
	\item Mantain the foundational logic of previous tools while implementing a more efficient data structure (such as a hierarchical JSON) to represent the Kernel’s responsibility distribution.
